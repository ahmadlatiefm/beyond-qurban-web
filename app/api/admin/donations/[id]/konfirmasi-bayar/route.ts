...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.donation.update({
    ...
    data: {
      paymentStatus: 'PAID',
      status: 'CONFIRMED',
      paymentProofUrl: body.buktiUrl?.trim() || donation.paymentProofUrl,
      paymentMethod: body.metodeBayar?.trim() || donation.paymentMethod,
      sisaPembayaran: 0,
    },
  })

  await createPengirimanFromPaidDonation(id)

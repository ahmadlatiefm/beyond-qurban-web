...
export async function POST(req: NextRequest) {
  ...
  const uploadDir = join(process.cwd(), 'public', 'uploads', 'laporan', 'blanko')
  const originalSize = file.size

  try {
    const result = await compressBlanko(bytes, join(uploadDir, filename))
    return NextResponse.json({
      url: `/uploads/laporan/blanko/${filename}`,
      width: result.width,
    ...
    console.error('laporan upload-blanko compress error', err)

...
import {
  ...
  faRotate, faExpand, faCamera, faMinus,
} from '@fortawesome/free-solid-svg-icons'
import {
  saveTemplateLaporan, deleteTemplateLaporan, toggleTemplateLaporanAktif,
} from '@/lib/actions/laporan-template'
import {
  AVAILABLE_LAPORAN_FIELDS, FOTO_ZONE_PRESETS,
  defaultField, defaultStaticText, defaultImageElement, defaultLineElement, defaultFotoZone,
  migrateLegacyElements,
  type LaporanElement, type LaporanFieldElement,
  type LaporanStaticTextElement, type LaporanImageElement,
  type LaporanImageKind, type LaporanLineElement, type LaporanFotoZoneElement,
  type FotoZoneKey, type FotoZoneLayout,
...
export default function LaporanTemplateClient({
  initialTemplates,
  ...
  function openEdit(tpl: Template) {
    setEditing({ ...tpl, elements: migrateLegacyElements(tpl.elements) })
    setMode('edit')
  }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await deleteTemplateLaporan(id)
  ...
  async function handleToggleAktif(tpl: Template) {
    try {
      await toggleTemplateLaporanAktif(tpl.id, !tpl.aktif)
      router.refresh()
      setTemplates(prev =>
        prev.map(t => {
          if (t.id === tpl.id) return { ...t, aktif: !tpl.aktif }
          if (!tpl.aktif) return { ...t, aktif: false }
          return t
        }),
      )
    ...
    <>
      <header className="sticky top-0 z-30 bg-brand-light/90 backdrop-blur-md border-b border-brand-muted/20 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          {mode === 'list' && (
            <Link href="/admin/laporan" className="text-brand-muted hover:text-brand-dark text-sm font-medium flex items-center gap-1.5">
              <FontAwesomeIcon icon={faArrowLeft} />
              Kembali
            </Link>
          )}
          <div>
            <h1 className="font-serif text-xl font-bold text-brand-dark">
              {mode === 'list' ? 'Template Laporan Penyaluran' : (editing?.id ? 'Edit Template' : 'Template Baru')}
            </h1>
            <p className="text-brand-muted text-xs mt-0.5">
              {mode === 'list' ? 'Upload blanko & susun zona foto + teks dengan drag & drop' : 'Atur zona foto, field dinamis & teks di atas blanko'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
    ...
    <>
      <div className="flex justify-between items-center mb-6">
        <p className="text-sm text-brand-muted">
          Template aktif akan otomatis dipakai saat generate PDF laporan per donatur.
        </p>
        <button
          onClick={onCreate}
          ...
          {templates.map(tpl => (
            <div key={tpl.id} className="bg-white border border-brand-muted/15 rounded-[14px] overflow-hidden flex flex-col">
              ...
              <div className="p-4 flex flex-col gap-2 flex-1">
                ...
                <p className="text-xs text-brand-muted">{tpl.elements.length} elemen dikonfigurasi</p>
...
function EditorView({
  ...
  const [elements, setElements] = useState<LaporanElement[]>(initial.elements)
  ...
  const placedFotoZoneKeys = useMemo(
    () => new Set(elements.filter(e => e.type === 'foto_zone').map(e => e.key)),
    [elements],
  )
  ...
  async function handleUpload(file: File) {
    ...
    setUploading(true)
    try {
      const res = await fetch('/api/admin/laporan/upload-blanko', { method: 'POST', body: fd })
  ...
  async function handleAssetUpload(file: File, kind: LaporanImageKind) {
    const fd = new FormData()
    fd.append('file', file)
    fd.append('kind', kind)
    setUploading(true)
    try {
      const res = await fetch('/api/admin/laporan/upload-asset', { method: 'POST', body: fd })
      const json = await res.json()
  ...
  function addImageElement(kind: LaporanImageKind, url: string, naturalW = 0, naturalH = 0) {
  ...
  function addStaticText() {
    ...
    const el = defaultStaticText(key, 50, 20)
    setElements(prev => [...prev, el])
    setSelectedKey(key)
  }

  function addLine() {
    const key = `line-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`
    const el = defaultLineElement(key, 20, 50)
    setElements(prev => [...prev, el])
    setSelectedKey(key)
  }

  function addFotoZone(zoneKey: FotoZoneKey, xPct?: number, yPct?: number) {
    if (placedFotoZoneKeys.has(zoneKey)) {
      onError('Zona foto ini sudah ditambahkan')
      return
    }
    const el = defaultFotoZone(zoneKey, zoneKey, xPct, yPct)
    setElements(prev => [...prev, el])
    setSelectedKey(zoneKey)
  }

  // ─── Drag from sidebar (new field or foto_zone)
  function onSidebarFieldDragStart(e: React.DragEvent, key: string) {
    e.dataTransfer.setData('field-key', key)
    e.dataTransfer.effectAllowed = 'copy'
  }

  function onSidebarFotoZoneDragStart(e: React.DragEvent, key: FotoZoneKey) {
    e.dataTransfer.setData('foto-zone-key', key)
    e.dataTransfer.effectAllowed = 'copy'
  }

  function onCanvasDragOver(e: React.DragEvent) {
    if (e.dataTransfer.types.includes('field-key') || e.dataTransfer.types.includes('foto-zone-key')) {
      e.preventDefault()
  ...
  function onCanvasDrop(e: React.DragEvent) {
    ...
    const fotoZoneKey = e.dataTransfer.getData('foto-zone-key') as FotoZoneKey | ''
    ...
    if (fotoZoneKey) {
      ...
      if (placedFotoZoneKeys.has(fotoZoneKey)) {
        onError('Zona foto ini sudah ditambahkan')
        return
      }
      addFotoZone(fotoZoneKey, xPct, yPct)
      return
    }
    ...
    if (ds.mode === 'move') {
      ...
      setElements(prev => prev.map(el => {
        ...
        if (el.type === 'foto_zone') {
          const newH = clamp(ds.origH + dyPct, 5, 100)
          return { ...el, width: newW, height: newH }
        }
        if (el.type === 'line') {
  ...
  function startDrag(
    ...
    const sizedTypes = ['image', 'foto_zone', 'line']
    dragStateRef.current = {
      ...
      origX: el.x,
      origY: el.y,
      origW: sizedTypes.includes(el.type) ? (el as LaporanImageElement | LaporanFotoZoneElement | LaporanLineElement).width : 0,
      origH: el.type === 'image' || el.type === 'foto_zone' ? (el as LaporanImageElement | LaporanFotoZoneElement).height : 0,
      aspect,
    }
  ...
  function updateElement(key: string, patch: Partial<LaporanElement>) {
    setElements(prev => prev.map(el => el.key === key ? ({ ...el, ...patch } as LaporanElement) : el))
  }
  ...
  async function generatePreview() {
    ...
    try {
      const res = await fetch('/api/admin/laporan/preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blankoUrl, elements }),
      })
      if (!res.ok) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        const res = await saveTemplateLaporan({
        ...
        onSaved({
          id: res?.id || initial.id || `tmp-${Date.now()}`,
          nama: nama.trim(),
          blankoUrl,
          width: imgSize.w,
          height: imgSize.h,
      ...
      <div className="bg-white border border-brand-muted/15 rounded-[14px] p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-3">
          ...
          <input
            ...
            placeholder="Laporan Penyaluran 2026"
        ...
        <div className="md:col-span-3 flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            ...
            <span>Set sebagai template aktif</span>
          </label>
          <span className="text-xs text-brand-muted">(otomatis menonaktifkan template lain)</span>
        </div>
      </div>
      ...
      {!blankoUrl ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_300px] gap-4">
          <div className="flex flex-col gap-4 min-w-0">
            <FotoZonePalette
              placedKeys={placedFotoZoneKeys}
              onAdd={addFotoZone}
              onDragStart={onSidebarFotoZoneDragStart}
            />
            ...
            <StaticTextPalette onAdd={addStaticText} onAddLine={addLine} />
          ...
          <div className="bg-white border border-brand-muted/15 rounded-[14px] p-4 flex flex-col gap-3 min-w-0">
              ...
              {elements.map(el => (
                <CanvasElement
                  ...
                  onChangeContent={content =>
                    updateElement(el.key, { content } as Partial<LaporanStaticTextElement>)
                  }
                />
              ))}
            </div>
            ...
            <p className="text-[11px] text-brand-muted">
              Tarik elemen untuk memindahkan. Klik untuk memilih dan atur dari panel kanan.
              Tarik handle pojok untuk resize. Zona foto akan menampilkan foto donatur saat PDF di-generate.
            </p>
          </div>
...
// ─── PALETTE: FOTO ZONE ─────────────────────────────────────────────────────
function FotoZonePalette({
  placedKeys, onAdd, onDragStart,
}: {
  placedKeys: Set<string>
  onAdd: (key: FotoZoneKey) => void
  onDragStart: (e: React.DragEvent, key: FotoZoneKey) => void
}) {
  return (
    <div className="bg-white border border-brand-muted/15 rounded-[14px] p-4">
      <h3 className="font-bold text-brand-dark text-sm mb-3 flex items-center gap-2">
        <FontAwesomeIcon icon={faCamera} />
        Zona Foto
      </h3>
      <div className="flex flex-col gap-2">
        {FOTO_ZONE_PRESETS.map(p => {
          const placed = placedKeys.has(p.key)
          return (
            <div
              key={p.key}
              draggable={!placed}
              onDragStart={e => onDragStart(e, p.key)}
              className={`border rounded-[10px] p-3 text-sm flex flex-col gap-1.5 ${placed ? 'bg-emerald-50 border-emerald-200' : 'bg-brand-light border-brand-muted/15 cursor-grab hover:bg-brand-light/70'}`}
            >
              <div className="flex items-center gap-2">
                <FontAwesomeIcon icon={faCamera} className="text-brand-muted" />
                <span className="font-bold text-brand-dark flex-1 truncate">{p.label}</span>
                {placed && <FontAwesomeIcon icon={faCheck} className="text-emerald-600 text-xs" />}
              </div>
              <p className="text-[10px] text-brand-muted">
                {placed ? 'Sudah di-kanvas — klik elemen untuk edit' : 'Tarik ke kanvas'}
              </p>
              {!placed && (
                <button
                  onClick={() => onAdd(p.key)}
                  className="text-[11px] font-bold border border-brand-muted/20 rounded-[8px] px-2 py-1.5 hover:bg-white flex items-center justify-center gap-1.5"
                >
...
function ImageElementsPalette({
  elements, uploading, onUpload,
}: {
  elements: LaporanElement[]
  uploading: boolean
  onUpload: (f: File, kind: LaporanImageKind) => void
}) {
  const placedKinds = new Set(
    elements.filter(e => e.type === 'image').map(e => (e as LaporanImageElement).imageKind),
  )
...
function ImageCard({
  icon, title, placed, uploading, accept, hint, onPick,
}: {
  icon: typeof faImage
  title: string
  placed: boolean
...
// ─── PALETTE: STATIC TEXT + LINE ────────────────────────────────────────────
function StaticTextPalette({ onAdd, onAddLine }: { onAdd: () => void; onAddLine: () => void }) {
  return (
    <div className="bg-white border border-brand-muted/15 rounded-[14px] p-4">
      <h3 className="font-bold text-brand-dark text-sm mb-3">Teks & Garis</h3>
      <div className="flex flex-col gap-2">
        <button
          onClick={onAdd}
...
function FieldPalette({
  placedFieldKeys, selectedKey, onSelect, onDragStart,
}: {
  ...
  return (
    <div className="bg-white border border-brand-muted/15 rounded-[14px] p-4">
      <h3 className="font-bold text-brand-dark text-sm mb-1">Field Dinamis</h3>
      <p className="text-[11px] text-brand-muted mb-3">Tarik ke kanvas untuk menambah field laporan</p>
      <div className="flex flex-col gap-2">
        {AVAILABLE_LAPORAN_FIELDS.map(f => {
...
function CanvasElement({
  ...
  if (element.type === 'foto_zone') {
    return (
      <div
        ...
        className={`absolute cursor-move flex items-center justify-center text-center px-2 ${selected ? 'ring-2 ring-brand-dark bg-brand-dark/5' : 'border-2 border-dashed border-brand-dark/40 bg-white/20 hover:bg-white/30'}`}
      >
        <div className="pointer-events-none">
          <FontAwesomeIcon icon={faCamera} className="text-brand-dark/60 text-2xl mb-1" />
          <div className="text-[11px] font-bold text-brand-dark/80 leading-tight">
            📷 {element.label}
          </div>
          <div className="text-[10px] text-brand-muted mt-0.5">
            max {element.maxFoto} · {layoutLabel(element.layout)}
          </div>
        </div>
        {selected && (
          <div
    ...
    return (
      <div
        ...
        style={{
          ...
          top: `${element.y}%`,
          width: `${element.width}%`,
          height: `${Math.max(0.3, element.height)}%`,
          backgroundColor: element.color,
          transform: element.rotation ? `rotate(${element.rotation}deg)` : undefined,
          transformOrigin: 'left center',
          minHeight: '2px',
        }}
        className={`absolute cursor-move ${selected ? 'outline outline-2 outline-brand-dark outline-offset-2' : 'hover:outline hover:outline-1 hover:outline-brand-dark/40'}`}
      >
        {selected && (
          <div
            onMouseDown={onResizeMouseDown}
            className="absolute -top-1.5 -right-1.5 w-3.5 h-3.5 bg-brand-dark border-2 border-white rounded-sm cursor-ew-resize z-10"
            title="Resize"
          />
        )}
      </div>
  ...
  const def = AVAILABLE_LAPORAN_FIELDS.find(a => a.key === element.key)
  return (
    <div
      onMouseDown={onMouseDown}
      onClick={e => { e.stopPropagation(); onSelect() }}
...
function layoutLabel(l: FotoZoneLayout): string {
  switch (l) {
    case 'grid_2x1': return 'Grid 2x1'
    case 'grid_2x2': return 'Grid 2x2'
    case 'kolom': return 'Kolom'
    case 'baris': return 'Baris'
  }
}

// ─── PROPERTIES: FOTO ZONE ──────────────────────────────────────────────────
function FotoZoneProperties({
  element, onChange, onDelete,
}: {
  element: LaporanFotoZoneElement
  onChange: (patch: Partial<LaporanFotoZoneElement>) => void
  onDelete: () => void
}) {
  const layouts: FotoZoneLayout[] = ['grid_2x1', 'grid_2x2', 'kolom', 'baris']
  return (
    <div className="flex flex-col gap-3 text-sm">
      <div>
        ...
        <div className="font-bold text-brand-dark">{element.label}</div>
        <div className="text-[10px] text-brand-muted">{element.key}</div>
      </div>

      <div>
        <div className="text-xs font-bold text-brand-muted mb-1">Label Zona</div>
        <input
          type="text"
          value={element.label}
      ...
      <div>
        <div className="flex justify-between items-center text-xs font-bold text-brand-muted mb-1">
          <span>Max Foto</span>
          <span>{element.maxFoto}</span>
        </div>
        <input
          type="range" min={1} max={6} step={1}
          value={element.maxFoto}
          onChange={e => onChange({ maxFoto: Number(e.target.value) })}
          className="w-full"
        />
      </div>

      <div>
        <div className="text-xs font-bold text-brand-muted mb-1">Layout</div>
        <div className="grid grid-cols-2 gap-1.5">
          {layouts.map(l => (
            <button
              key={l}
              onClick={() => onChange({ layout: l })}
              className={`text-xs px-2 py-1.5 rounded-[8px] border ${element.layout === l ? 'bg-brand-dark text-brand-accent-light border-brand-dark' : 'border-brand-muted/20 hover:bg-brand-light'}`}
            >
              {layoutLabel(l)}
            </button>
          ))}
      ...
      <div>
        <label className="flex items-center gap-2 text-xs font-bold text-brand-muted cursor-pointer">
          <input
            type="checkbox"
      ...
      {element.border && (
        <div>
          <div className="text-xs font-bold text-brand-muted mb-1">Warna Border</div>
          <div className="flex items-center gap-2">
            <input
              type="color"
      ...
      <div>
        <div className="flex justify-between items-center text-xs font-bold text-brand-muted mb-1">
          <span>Gap Antar Foto</span>
          <span>{element.gap.toFixed(1)}%</span>
        </div>
        <input
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <div>
          <div className="font-bold text-brand-muted mb-1">Lebar</div>
          <input
            type="number" min={5} max={100} step={0.1}
            value={Number(element.width.toFixed(1))}
            onChange={e => onChange({ width: clamp(Number(e.target.value), 5, 100) })}
            className="w-full border border-brand-muted/20 rounded-[8px] px-2 py-1.5"
          />
        </div>
        <div>
          <div className="font-bold text-brand-muted mb-1">Tinggi</div>
          <input
            type="number" min={5} max={100} step={0.1}
            value={Number(element.height.toFixed(1))}
            onChange={e => onChange({ height: clamp(Number(e.target.value), 5, 100) })}
            className="w-full border border-brand-muted/20 rounded-[8px] px-2 py-1.5"
          />
        </div>
      </div>
...
function LineProperties({
  ...
  element: LaporanLineElement
  onChange: (patch: Partial<LaporanLineElement>) => void
  onDelete: () => void
}) {
  return (
    <div className="flex flex-col gap-3 text-sm">
...
function TextProperties({
  ...
  element: LaporanFieldElement | LaporanStaticTextElement
  onChange: (patch: Partial<LaporanElement>) => void
  onDelete: () => void
}) {
  const isStatic = element.type === 'static_text'
  const label = isStatic
    ? 'Teks Statis'
    : (AVAILABLE_LAPORAN_FIELDS.find(a => a.key === element.key)?.label || element.key)
    ...
    <div className="flex flex-col gap-3 text-sm">
      ...
      {isStatic && (
        <div>
          ...
          <textarea
            value={(element as LaporanStaticTextElement).content}
            onChange={e => onChange({ content: e.target.value } as Partial<LaporanStaticTextElement>)}
            rows={3}
            className="w-full border border-brand-muted/20 rounded-[8px] px-2 py-1.5 text-xs"
      ...
      <div>
        ...
        <input
          ...
          onChange={e => onChange({ fontSize: Number(e.target.value) } as Partial<LaporanElement>)}
          className="w-full"
        />
      </div>
      <div>
        ...
        <div className="flex gap-1.5">
          {(['normal', 'bold'] as const).map(w => (
            <button
              key={w}
              onClick={() => onChange({ fontWeight: w } as Partial<LaporanElement>)}
      ...
      <div>
        ...
        <div className="flex gap-1.5">
          {[
            ...
            <button
              key={a.v}
              onClick={() => onChange({ align: a.v as LaporanFieldElement['align'] } as Partial<LaporanElement>)}
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <input
            type="color"
            value={element.color}
            onChange={e => onChange({ color: e.target.value } as Partial<LaporanElement>)}
            className="w-12 h-9 border border-brand-muted/20 rounded-[8px] cursor-pointer"
          />
          <input
            type="text"
      ...
      <div>
        ...
        <select
          ...
          onChange={e => onChange({ fontFamily: e.target.value } as Partial<LaporanElement>)}
          className="w-full border border-brand-muted/20 rounded-[8px] px-2 py-1.5 text-xs"
        >
      ...
      <div>
        ...
        <input
          ...
          onChange={e => onChange({ maxWidth: Number(e.target.value) || undefined } as Partial<LaporanElement>)}
          className="w-full"
        />
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          ...
          <input
            type="number" min={0} max={100} step={0.1}
            value={Number(element.x.toFixed(1))}
            onChange={e => onChange({ x: clamp(Number(e.target.value)) } as Partial<LaporanElement>)}
            className="w-full border border-brand-muted/20 rounded-[8px] px-2 py-1.5"
          />
        </div>
        <div>
          ...
          <input
            type="number" min={0} max={100} step={0.1}
            value={Number(element.y.toFixed(1))}
            onChange={e => onChange({ y: clamp(Number(e.target.value)) } as Partial<LaporanElement>)}
            className="w-full border border-brand-muted/20 rounded-[8px] px-2 py-1.5"
          />
        </div>
      </div>
...
function ImageProperties({
  ...
  element: LaporanImageElement
  onChange: (patch: Partial<LaporanImageElement>) => void
  onDelete: () => void
}) {
  const kindLabel: Record<LaporanImageKind, string> = {

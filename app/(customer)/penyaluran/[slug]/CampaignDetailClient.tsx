...
export default function CampaignDetailClient({ campaign }: Props) {
  ...
  const checkoutUrl = `/penyaluran/checkout?campaign=${campaign.slug}&qty=${qty}&type=${isSedekahOnly ? 'sedekah' : donationType}`
  ...
  return (
    <div className="bg-white rounded-[16px] border border-brand-muted/10 shadow-premium overflow-hidden sticky top-28">
      ...
      <div className="p-5 bg-brand-surface text-brand-light">
        ...
        <div className="font-serif text-2xl font-bold text-brand-accent">{formatCurrency(campaign.price)}<span className="text-sm font-sans font-normal text-brand-accent-light/70">/ekor</span></div>
      </div>
      ...
      <div className="p-5 flex flex-col gap-4">
        ...
        >
          ...
          {isSedekahOnly ? 'Sedekah Sekarang' : 'Donasi Sekarang'}

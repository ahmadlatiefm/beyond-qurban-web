...
export default function Header({ storeName, navItems, ctaText, ctaHref, logoUrl }: Props) {
  ...
  const brandName = storeName || 'Beyond Qurban'
  const btnText = ctaText || 'Beli Sekarang'
  const btnHref = ctaHref || '/katalog'
  ...
  return (
    <>
      ...
      >
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {links.map((link) => (
              <Link key={link.href} href={link.href}
                className={isActive(pathname, link.href)
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link href={btnHref} className="bg-cta-gradient text-brand-text-dark font-bold text-sm py-2.5 px-6 rounded-[20px] shadow-premium hover:scale-105 transition-transform">
              {btnText}
            </Link>
          </div>

          {/* Mobile hamburger */}
          <button className="md:hidden text-brand-light hover:text-brand-accent transition-colors" onClick={() => setMenuOpen(true)} aria-label="Open menu">
      ...
      {/* Mobile drawer */}
      <div className="fixed inset-0 z-[60] bg-brand-dark flex flex-col md:hidden"
        ...
        <div className="px-6 h-20 flex items-center justify-between border-b border-brand-surface-light/30">
          <span className="font-serif text-brand-accent text-xl font-bold tracking-wide">{brandName}</span>
          <button className="text-brand-light" onClick={() => setMenuOpen(false)} aria-label="Close menu">
        ...
        <nav className="flex-1 flex flex-col pt-8 px-6 gap-6">
          {links.map((link) => (
            <Link key={link.href} href={link.href}
              className={`font-medium text-xl border-b border-brand-surface-light/20 pb-4 transition-colors ${isActive(pathname, link.href) ? 'text-brand-accent' : 'text-brand-light'}`}
        ...
        <div className="p-6">
          <Link href={btnHref} className="w-full bg-cta-gradient text-brand-text-dark font-bold text-lg py-4 rounded-[20px] shadow-premium flex items-center justify-center"
            onClick={() => setMenuOpen(false)}>
            {btnText}
          </Link>
        </div>
